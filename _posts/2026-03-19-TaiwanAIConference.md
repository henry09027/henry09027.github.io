...
的
BusinessRelationships Analytics
（
BRA
）
用
專利演算法把「看不見」的供應商
‑
客戶金額變成可量化的數字，讓投資人能即時掌握供應鏈動能，從而在全球市場中取得超額報酬（
Alpha
）。
BRA
背後的魔法：快速說明演算法與資料來源（監管財報），如何推算出隱藏的供應鏈金額。
價格傳遞的「波浪」：客戶或供應商的價格變動會向上（供應商）或向下（客戶）傳遞，產生可捕捉的投資動能。
實戰投資佈局：示範如何用
BRA
在美股、日股、台股構建投資組合，並展示真實的超額報酬案例。
即時互動
Q&A
：現場解答您對
BRA
...
<script>
  ...
  const pdfUrl = encodeURI('{{ "/assets/images/etc/TaiwanAIConference.pdf" | relative_url }}');
